(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".partners-border__slider")) new core(".partners-border__slider", {
            modules: [],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
    ...
    da.init();
    let shortStr = document.querySelectorAll(".item-festival__text p"), headerIcon = document.querySelector(".search-header__icon"), script_showMore = document.querySelector(".reviews__button"), reviewCardsLength = document.querySelectorAll(".item-review").length, goTopBtn = document.querySelector(".footer__button"), mainBtn = document.querySelector(".main-block__button");
    shortStr.forEach((p => {
        if (p.textContent.length > 65) p.textContent = `${p.textContent.substring(0, 64)}...`;
    }));
    headerIcon.addEventListener("click", (function(e) {
        let headerRequests = document.querySelector(".search-header__requests");
        headerRequests.classList.toggle("_active");
    }));
    let items = 3;
    if (script_showMore != null) script_showMore.addEventListener("click", (() => {
        items += reviewCardsLength;
        const arr = Array.from(document.querySelector(".reviews__items").children);
        const visibleCards = arr.slice(0, items);
        visibleCards.forEach((item => item.classList.toggle("is-visible")));
        if (script_showMore.innerHTML.length >= 9) script_showMore.innerHTML = "Згорнути"; else script_showMore.innerHTML = "Показати ще";
    }));
    ...
    document.documentElement.addEventListener("click", (e => {
        if (!e.target.closest(".actions-header")) {
            let headerRequests = document.querySelector(".search-header__requests");
            headerRequests.classList.remove("_active");
        }
    }));
    ...
    itemsTab.forEach(((item, index) => {
        item.addEventListener("click", (e => {
            handleIndicator(e.target);
        }));
        item.classList.contains("is-active") && handleIndicator(item);
    }));
    if (mainBtn != null) mainBtn.addEventListener("click", goTo);
    document.querySelector("#search-header__list").oninput = function() {
        let val = this.value.trim().toLowerCase();
        let searchItems = document.querySelectorAll(".search-header__list li");
        document.querySelector(".search-header__popular").classList.add("hide");
        if (val != "") searchItems.forEach((function(elem) {
            if (elem.innerText.toLowerCase().search(val) >= 0) {
            ...
            document.querySelector(".search-header__popular").classList.remove("hide");
            searchItems.forEach((function(elem) {
                if (elem.innerText.search(val) >= 0) {
                    elem.classList.add("hide");
                    elem.innerHTML = elem.innerText;
                }
            }));
    ...
    function insertMark(req, pos, len) {
        return req.slice(0, pos) + "<mark>" + req.slice(pos, pos + len) + "</mark>" + req.slice(pos + len);
    }
    try {
        document.querySelector("#new").addEventListener("click", (() => {
            document.querySelector(".spollers__title").innerText = "Найновіші";
            mySort();
        }));
        document.querySelector("#old").addEventListener("click", (() => {
            document.querySelector(".spollers__title").innerText = "Найстаріші";
            mySortDesc();
        }));
        function mySort() {
            let tabItems = document.querySelectorAll(".blog__items");
            tabItems.forEach((item => {
                for (let i = 0; i < item.children.length; i++) for (let j = i; j < item.children.length; j++) if (+item.children[i].getAttribute("data-date") < +item.children[j].getAttribute("data-date")) {
                    let replacedNode = item.replaceChild(item.children[j], item.children[i]);
                    insertAfter(replacedNode, item.children[i]);
                }
            }));
        }
        function mySortDesc() {
